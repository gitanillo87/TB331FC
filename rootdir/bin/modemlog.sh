...
TABLET=$(getprop ro.vendor.config.zui.devicetype)
...
elif [ -e /data/local/newlog/cs.cfg ]; then
#    setprop "persist.sys.lenovo.log.qxdmcfg" "/sdcard/log_cfg/cs.cfg"
    CFGFILE=/data/local/newlog/cs.cfg
    Log "[diag] setprop persist.sys.lenovo.log.qxdmcfg to /data/local/newlog/cs.cfg"
else
...
elif [ $(getprop ro.vendor.config.zui.devicetype) = PAD ]; then
    /vendor/bin/diag_mdlog -s 512 -n 10 -f $CFGFILE -m $CFGFILE -o /data/vendor/diag_mdlog
    Log "for wifi tablet"
#JLINDEN-5142 enable qdss for Linden   BEGIN
elif [ $(getprop ro.vendor.config.zui.devicetype) = PAD_WITH_SIM ]; then
    /vendor/bin/diag_mdlog -q 2 -u -s 512 -n $CFGNUM -f $CFGFILE -m $CFGFILE -i -o $LOGFILE
    Log "enable linden qdss of qssi"
#JLINDEN-5142 enable qdss for Linden   END
else
